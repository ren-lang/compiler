...
function reduceIndent(targetIndent, finalIndent, code) {
    ...
    for (
        var i = 0;
        i < lines.length && (lines[i] == '' || lines[i].startsWith(targetIndent));
        i++
    ) {
...
function mainForCommandLine() {
    ...
    for (let i = 2; i < process.argv.length; i++) {
        ...
        ;((
            {
                '--output': () => {
                    outputFileOption = arg
                    i++
                },
                '-': () => {
                    inputIsStdin = true
                }, // Specifying - as a file name commonly means stdin
                '--stdin': () => {
                    inputIsStdin = true
                },
                '--validate': () => {
                    assert_(false, '--validate is not supported.')
                },
                '--yes': () => {},
            }[option] ||
            (() => {
                inputFiles.push(option)
            })
        )())
    }

    assert_(inputFiles.length >= 1 || inputIsStdin, 'I need an input file or --stdin')

    assert_(
        !outputFileOption || inputFiles.length === 1 || inputIsStdin,
        'Cannot specify --output with multiple files',
    )

    for (const inputFile of inputFiles) {
        console.log(inputFile)

        let outputFile = outputFileOption
        if (!inputIsStdin && !outputFileOption) {
            outputFile = inputFile
        }

        const inputStream = inputIsStdin
            ? process.stdin
            : // TODO YOLO
        ...
        let result = cp.spawnSync('npm', ['exec', 'elm-format', '--', '--stdin'], {
